# Repo tooling (unrelated to the advocacy app)

This repository started as a place to track a set of Claude Code memory /
context tools set up in a Claude Code on the web session, before the
Alpha Nodus Advocacy app was built here. Because that session ran in an
ephemeral cloud container, anything meant to persist was committed here.
This file is that original tracking note, preserved as-is.
