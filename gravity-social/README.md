...
- **Every platform gets its own piece.** One weekly angle becomes a LinkedIn document carousel, a founder story for Shamit, an X thread and an Instagram reel, each following its playbook in `platforms/` and the hard limits in `platforms/limits.yaml`. Cross-posting the same text is blocked.
...
- **Results are measured in pipeline.** Every link carries its post id (`utm_content`), so a demo request or a free "your exam, itemized" request traces back to the post that earned it (`reports/attribution.md`).
...
| "Approve 2026-W40-li-co-02 and all the X posts" | `gate.py approve …` in your name |
...
- 14 drafts in `content/queue/pending/`: 4 LinkedIn company (poll, the five-tests document carousel, the Surest post, the agentic-AI post), 2 for Shamit, 5 X (including a 7-part thread), 3 Instagram (2 carousels, 1 reel script). All pass lint with 0 errors and 0 warnings. **None are approved.**
...
| # | What | Where |
|---|---|---|
| 1 | Connect LinkedIn (company page), X and Instagram to Metricool brand `6979710` | app.metricool.com → Brand → Connections |
| 2 | Add approvers besides Shamit, and Metricool reviewer emails | `config.yaml` → `approval` |
...
| 6 | The website form / CRM export carrying `utm_content`, so demo requests can be attributed | `reports/attribution.md` |
